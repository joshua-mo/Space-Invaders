...
Game::GameScreen::GameScreen() {
    ...
    if (score == 0)               //nach Reset wieder 1:!
    {           
        activeEnemies = 1;
        activeEnemies2 = 1;
    }
    ...
    for (int i = 0; i < NUM_MAX_ENEMIES; i++)
    {
        ...
        enemy2[i].rect.x = GetRandomValue(0, 472);
...
void Game::GameScreen::Update() {
 ...
 if ((player.rect.x + 54) >= 600) player.rect.x = 600 - 54;           //Spieler an Wänden einschränken
 ...
 for (int i = 0; i < NUM_SHOOTS; i++)
 {
     if (bullet[i].active)
     {
         ...
         for (int j = 0; j < activeEnemies || j < activeEnemies2;  j++)
         {
             if (enemy[j].active || enemy2[j].active)
             {
                 ...
                 if (enemy2[j].gothit == true)
                 {
                     ...
                     {
                         bullet[i].active = false;
                         enemy2[j].rect.x = GetRandomValue(0, 472);      //Spawnbereich neuer Gegner x
         ...
         for (int j = 0; j < activeEnemies || j < activeEnemies2; j++)
         {
             if (enemy[j].active | enemy2[j].active)
             {
                 ...
                 {
                     player.lives--;
                     enemy2[j].rect.x = GetRandomValue(0, 536);      //Spawnbereich neuer Gegner x
    ...
    for (int i = 0; i < activeEnemies || i < activeEnemies2; i++)
    {
        ...
        if (enemy2[i].active)
        {
            ...
            {
                enemy2[i].rect.x = GetRandomValue(0, 600 - 100);
                enemy2[i].rect.y = GetRandomValue(-1000, -20);
                planet.landed--;
...
void Game::GameScreen::Draw() {
    ...
    DrawTexture(background, 0, 0, WHITE); 
     ...
     //Anzeigen
     DrawText(" M - pause", 20, 20, 20, LIGHTGRAY);
     ...
     DrawText(TextFormat("Score: %02i", highscore), 425, 140, 35, LIGHTGRAY);
     //Live Anzeige
     if (player.lives == 3) {
